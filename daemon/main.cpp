...
bool Daemon::handle_file_chunk_env(Client *client, Msg *msg)
{
    ...
    if (msg->type == M_FILE_CHUNK && client->pipe_to_child >= 0) {
        ...
        while (len) {
            // trace()<< "handle_file_chunk_env: Writing to client->pipe_to_child "<< client->pipe_to_child<< " Length "<< len<< endl;
            ...
            off += bytes;
            if (bytes < 100000)
                trace() << "PID: " << client->child_pid << " Finished environment transfer for "<< client->outfile << endl;
        }
    ...
    if (msg->type == M_END) {
        ...
        close(client->pipe_to_child);
        client->pipe_to_child = -1;
        trace() << "PID: " << client->child_pid << " Received message M_END for "<< client->outfile <<endl;
...
int main(int argc, char **argv)
{
    ...
    while (true) {
        ...
        const int c = getopt_long(argc, argv, "N:n:m:l:s:hvdb:u:p:i:", long_options, &option_index);
